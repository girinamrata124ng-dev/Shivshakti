...
public class BillServiceImpl implements BillService {
    ...
    // Font path - will be loaded lazily for each PDF
    private String marathiFontPath = null;

    private String getMarathiFontPath() throws IOException {
        if (marathiFontPath == null) {
            // Check if font exists in classpath
            try {
                InputStream fontStream = getClass().getClassLoader().getResourceAsStream("fonts/NotoSansDevanagari-Regular.ttf");
                if (fontStream != null) {
                    marathiFontPath = "fonts/NotoSansDevanagari-Regular.ttf";
                    fontStream.close();
                } else {
                    marathiFontPath = null;
                }
            } catch (Exception e) {
                log.warn("Could not find Marathi font: {}", e.getMessage());
                marathiFontPath = null;
            }
        }
        return marathiFontPath;
    }

    private PdfFont createMarathiFont() throws IOException {
        String fontPath = getMarathiFontPath();
        if (fontPath != null) {
            try {
    ...
    private void generateBeautifulPdf(String filePath,
                                      Integer bill,
                                      String name,
        ...
        PdfFont font = createMarathiFont();
        ...
        // Load image as bytes to avoid "Pdf indirect object belongs to other PDF document" error
        byte[] imageBytes = IOUtils.toByteArray(
                getClass().getClassLoader().getResourceAsStream("images/background_bill.jpg"));
        canvas.addImageFittedIntoRectangle(
                ImageDataFactory.create(imageBytes),
                new com.itextpdf.kernel.geom.Rectangle(0, 0, 595, 842),
                false
        );
        ...
        for (int i = 0; i < items.length(); i++) {
            ...
            Map<String, Object> p =
                    CafeUtils.getMapFromJson(items.getString(i));

            float y = startY - (i * rowGap);

            drawText(canvas, String.valueOf(i + 1), colX[0], y, 16, font); // अ.क्र.
            drawText(canvas, getValue(p, "name"), colX[1], y, 16, font);  // डाळीचे नाव
            drawText(canvas, getValue(p, "quantity"), colX[2], y, 16, font); // वजन
            drawText(canvas, getValue(p, "plus"), colX[4], y, 16, font); // डाळ
            drawText(canvas, getValue(p, "waste"), colX[5], y, 16, font);// चुनी
            drawText(canvas, getValue(p, "wastage2"), colX[6], y, 16, font);// तुट/तुकडा
            drawText(canvas, getValue(p, "price"), colX[3], y, 16, font);// दर
            drawText(canvas, getValue(p, "total"), colX[7], y, 16, font);// एकूण
        }
    ...
    // Helper method to draw text at specific position
    private void drawText(PdfCanvas canvas, String text, float x, float y, float fontSize, PdfFont font) {
        canvas.beginText()
                .setFontAndSize(font, fontSize)
                .moveText(x, y)
                .showText(text != null ? text : "-")
                .endText();
    }
